...
init(Ref, Socket, Transport, _Opts) ->
  ...
  State = #state{
    socket            = Socket,
    transport         = Transport,
    peer_name         = maxwell_protocol_channel_lib:peer_chan_name(PeerIP),
...
handle_info({tcp, Socket, Data}, State = #state{
  ...
  case proc_msg(Data, PeerName) of
    noreply ->
      {noreply, State, ?TIMEOUT};
    {reply, {ok,BinRep}} when is_binary(BinRep) ->
      ...
      Response = [?RESPONSE, <<Len:32>> | OkReply],
...
proc_msg(Data, From) when is_binary(Data) ->
  proc_msg(maxwell_protocol_channel_pb:decode(chan_msg_t, Data), From);
proc_msg(#chan_msg_t{rpc_type = 'CAST',msg_type = Type, payload = PayLoad}, From) ->
  case maxwell_protocol_channel_handler_manager:find_handler(Type) of
    {ok,Handler} ->
      case catch(Handler(cast,PayLoad, From)) of
        {'EXIT',Reason} ->
  ...
  noreply;
proc_msg(#chan_msg_t{rpc_type = 'CALL',msg_type = Type, payload = PayLoad}, From) ->
  Reply = case maxwell_protocol_channel_handler_manager:find_handler(Type) of
    {ok,Handler} ->
      case catch(Handler(call,PayLoad, From)) of
        {'EXIT',Reason} ->
          {error,Reason};
